                 ...
                 unsigned char *image, int h, int w, int c, float alpha)
{
    ...
    float *ver_raw = (float *)calloc(3 * nver, sizeof(float));
    ...
    for (int i = 0; i < nver; ++i)
    {
        ver_raw[3 * i] = vertices[3 * i];
        ver_raw[3 * i + 1] = vertices[3 * i + 1];
        ver_raw[3 * i + 2] = vertices[3 * i + 2];
        ...
        nx = light[0] - vertices[3 * i];
        ny = light[1] - vertices[3 * i + 1];
        nz = light[2] - vertices[3 * i + 2];
        ...
        cos_sum = vertices[3 * i] + vertices[3 * i + 1] + vertices[3 * i + 2];

        vertices[3 * i] = cos_sum * directional[0] + ambient[0];
        vertices[3 * i + 1] = cos_sum * directional[1] + ambient[1];
        vertices[3 * i + 2] = cos_sum * directional[2] + ambient[2];

        // for (int j = 0; j < 3; ++j)
        // {
        //     float tmp = cos_sum * directional[j] + ambient[j];
        //     vertices[3 * i + j] = min(max(tmp, 0), 1);
        // }
    }

    _rasterize(image, ver_raw, triangles, vertices, ntri, h, w, c, alpha);
